...
class DiffusionSampler(nn.Module):
    ...
    def _euler(self, model, x_start, SDE=False, record=False, verbose=False):
        ...
        for step in pbar:
            ...
            # record
            if record:
                self._record(x, score, epsilon, sigma, factor)
        return x

    def _record(self, x, score, epsilon, sigma, factor):
        """
...
class DAPS(nn.Module):
    ...
    def sample(self, model, x_start, operator, measurement, evaluator=False, record=False, verbose=False):
        """
